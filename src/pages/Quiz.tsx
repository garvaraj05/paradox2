...
const Quiz: React.FC = () => {
    ...
    return (
      <div className="min-h-screen pt-16 bg-black flex items-center justify-center p-4 relative overflow-hidden">
        ...
        <motion.div 
          ...
          className="max-w-4xl w-full bg-zinc-900/80 backdrop-blur-xl border border-white/10 rounded-3xl p-8 md:p-12 relative z-10 shadow-[0_0_50px_rgba(0,0,0,0.5)]"
        >
          <div className="flex items-center mb-6 text-neon-purple">
            <AlertTriangle className="w-8 h-8 mr-3" />
            <h1 className="text-2xl md:text-3xl font-mono font-bold tracking-tighter">OPERATION PARADOX</h1>
          </div>
          
          <h2 className="text-4xl md:text-6xl font-black text-white mb-6 leading-tight font-display">
          ...
          <div className="space-y-4 text-gray-300 text-lg md:text-xl leading-relaxed mb-10 border-l-4 border-neon-cyan pl-6 font-light">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-10">
            {[
              ...
              <motion.div 
                ...
                className="bg-black/40 p-5 rounded-xl border border-white/10 hover:border-neon-purple/50 transition-colors"
              >
                <span className="block text-gray-500 text-xs font-mono uppercase tracking-wider mb-1">{stat.label}</span>
                <span className="text-2xl md:text-3xl text-white font-display font-bold">{stat.val}</span>
              </motion.div>
            ))}
          </div>
          ...
          <motion.button 
            ...
            className="w-full bg-neon-purple hover:bg-purple-600 text-white font-bold py-5 rounded-xl text-2xl font-display tracking-widest transition-all shadow-[0_0_25px_rgba(176,38,255,0.4)] hover:shadow-[0_0_40px_rgba(176,38,255,0.6)] flex items-center justify-center group"
          >
            START MISSION
        ...
        <div className="max-w-3xl w-full text-center">
            ...
            >
                <motion.div 
                    ...
                    className="w-28 h-28 bg-neon-cyan/10 rounded-full flex items-center justify-center mx-auto mb-8 border border-neon-cyan shadow-[0_0_20px_rgba(0,243,255,0.3)]"
                >
                    <Brain className="w-14 h-14 text-neon-cyan" />
                </motion.div>
                <h2 className="text-4xl md:text-6xl font-bold text-white mb-6 font-display tracking-tight">MISSION STATUS: LOGGED</h2>
                <p className="text-gray-400 text-xl mb-10 leading-relaxed max-w-lg mx-auto">
                    Your responses have been encrypted and sent to the mainframe. The Paradox is analyzing your logic patterns.
                </p>
                <div className="p-6 bg-white/5 rounded-2xl border border-white/10 mb-10 max-w-lg mx-auto">
                    <p className="text-neon-cyan font-mono text-lg">Agent: {user?.name}</p>
                    <p className="text-base text-gray-500 mt-2">Results will be announced on the dashboard.</p>
                </div>
                <motion.button 
                    whileHover={{ scale: 1.05, backgroundColor: "rgba(255, 255, 255, 0.2)" }}
                    whileTap={{ scale: 0.95 }}
                    onClick={() => navigate('/')}
                    className="px-10 py-4 bg-white/10 text-white font-semibold text-lg rounded-xl transition-all border border-white/10 hover:border-white/30"
                >
                    Return to Base
                </motion.button>
            </motion.div>
  ...
  return (
    <div className="min-h-screen pt-20 bg-black text-white relative overflow-hidden flex flex-col">
        ...
        {/* Sticky Header */}
        <div className="fixed top-20 left-0 w-full bg-black/80 backdrop-blur-lg border-b border-white/10 z-40">
            <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex justify-between items-center">
                ...
                <div className="text-gray-400 text-sm font-mono bg-white/5 px-4 py-1.5 rounded-full border border-white/10">
        ...
        {/* Main Content */}
        <div className="flex-grow flex items-center justify-center p-4 mt-20 pb-16">
            <div className="max-w-6xl w-full">
                <AnimatePresence mode='wait'>
                    <motion.div
                        ...
                        className="bg-zinc-900/60 border border-white/10 p-8 md:p-14 rounded-[2rem] relative backdrop-blur-md shadow-2xl overflow-hidden"
                    >
                        ...
                        <div className="relative z-10">
                            <div className="flex justify-between items-start mb-8">
                                <span className="inline-flex items-center px-4 py-1.5 bg-neon-cyan/10 text-neon-cyan text-sm font-mono font-bold rounded-full border border-neon-cyan/20">
                            ...
                            <motion.h3 
                              layout
                              className="text-2xl md:text-4xl lg:text-5xl font-bold mb-12 leading-snug font-display text-white min-h-[6rem] flex items-center"
                            >
                                {currentQ.question}
                            </motion.h3>
                                ...
                                {currentQ.options.map((option, index) => {
                                    ...
                                    return (
                                        <motion.button
                                            ...
                                            whileTap={{ scale: 0.98 }}
                                            className={`relative w-full text-left p-6 md:p-8 rounded-2xl border-2 transition-all duration-200 flex items-center group overflow-hidden ${
                                                isSelected
                                                ? 'bg-neon-purple/20 border-neon-purple text-white shadow-[0_0_20px_rgba(176,38,255,0.3)]'
                                                : 'bg-black/60 border-gray-800 text-gray-300 hover:border-gray-500 hover:bg-white/5'
                                            }`}
                                        >
                                            <div className={`w-8 h-8 rounded-full border-2 flex items-center justify-center mr-6 flex-shrink-0 transition-colors ${
                                                isSelected
                                            ...
                                            <span className={`font-sans text-xl md:text-2xl ${isSelected ? 'font-bold' : 'font-medium'}`}>
                                                {option}
                                            </span>
                                        </motion.button>
                                    );
                            ...
                            <div className="mt-12 flex justify-end border-t border-white/10 pt-8">
                                <motion.button
                                    ...
                                    className={`px-12 py-5 rounded-xl font-bold transition-all flex items-center text-xl font-display tracking-wide ${
                                        answers[currentQ.id]
